...
(function () {
  ...
  function mainPy(s) {
    ...
    return [
      ...
      '',
      'STARTED = time.time()',
      'METRICS = {"requests": 0, "by_status": {}, "by_method": {}}',
      'LOG_SILENT = os.environ.get("LOG") == "silent" or os.environ.get("PYTEST_CURRENT_TEST")',
      '',
      'def _metrics_text():',
      '    up = time.time() - STARTED',
      '    out = "# TYPE app_uptime_seconds gauge\\napp_uptime_seconds %.1f\\n" % up',
      '    out += "# TYPE app_requests_total counter\\napp_requests_total %d\\n" % METRICS["requests"]',
      '    for k, v in METRICS["by_status"].items():',
      '        out += \'app_responses_total{class="%s"} %d\\n\' % (k, v)',
      '    for k, v in METRICS["by_method"].items():',
      '        out += \'app_requests_by_method_total{method="%s"} %d\\n\' % (k, v)',
      '    return out',
      '',
      ...
      '',
      '    def _observe(self, method, path, code, t0):',
      '        METRICS["requests"] += 1',
      '        METRICS["by_method"][method] = METRICS["by_method"].get(method, 0) + 1',
      '        cls = "%dxx" % (code // 100)',
      '        METRICS["by_status"][cls] = METRICS["by_status"].get(cls, 0) + 1',
      '        if not LOG_SILENT:',
      '            rec = {"t": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "level": "error" if code >= 500 else "info",',
      '                   "msg": "request", "method": method, "path": path.split("?")[0], "status": code, "ms": int((time.time() - t0) * 1000)}',
      '            try: print(json.dumps(rec), flush=True)',
      ...
      '        try: self._observe(self.command, self.path, code, getattr(self, "_t0", time.time()))',
      '        except Exception: pass',
      '',
      '    def _body(self):',
      ...
      '        try:',
      '            if u.path == "/healthz":',
      '                return self._send(200, {"ok": True, "uptime_s": int(time.time() - STARTED)})',
      '            if u.path == "/readyz":',
      '                try:',
      '                    ' + (res[0] ? 'svc_' + res[0].name + '.list_({"limit": "1"}, None)' : (s.auth ? 'db.listing("users", {}, 1, 0)' : 'db.migrate()')),
      '                    return self._send(200, {"ready": True})',
      '                except Exception as e:',
      '                    return self._send(503, {"ready": False, "error": str(e)})',
      '            if u.path == "/metrics":',
      '                return self._send(200, _metrics_text(), "text/plain; version=0.0.4")',
