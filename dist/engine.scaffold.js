...
(function () {
  ...
  function normalize(spec) {
    var s = S().normalizeSpec(spec || DEMO_SPEC);
    if (s.auth) {
      // prepend the auth entities (user, session) unless already present
      var names = s.entities.map(function (e) { return e.name; });
      var authEnts = A().entities().map(function (e) { return S().normalizeSpec({ name: s.name, entities: [e] }).entities[0]; });
      s.entities = authEnts.filter(function (e) { return names.indexOf(e.name) < 0; }).concat(s.entities);
    }
    return s;
  }
  ...
  /* ---- frontend ---- */
  function frontend(s) {
    var forms = s.entities.filter(function (e) { return e.name !== 'user' && e.name !== 'session'; }).map(function (e) {
  ...
  function tests(s) {
    var out = {};
    var first = s.entities.filter(function (e) { return e.name !== 'user' && e.name !== 'session'; })[0];
  ...
  function meta(s) {
    ...
    return {
      'package.json': JSON.stringify({ name: s.name, version: '0.1.0', private: true,
        description: 'Generated by CodeSovereign — full-stack, dependency-free.', scripts: scripts }, null, 2) + '\n',
  ...
  function generate(spec) {
    ...
    s.entities.filter(function (e) { return e.name !== 'user' && e.name !== 'session'; }).forEach(function (e) {
