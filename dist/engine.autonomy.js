/* =====================================================================
   engine.autonomy.js  —  Engine.Autonomy   (GodMode blueprint §69)
   ...
   so the user picks how much runs without a prompt.
     ...
     godmode   — + package + deploy + release, fully autonomous
...
(function () {
  ...
  var LEVELS = ['assist', 'build', 'engineer', 'autopilot', 'godmode'];
  var CAPS = {
    ...
    godmode:   { write: true,  generate: true,  command: true,  repair: true,  observe: true,  deploy: true,  release: true,  network: true }
  };

  function get() {
    try { var v = localStorage.getItem(KEY); if (LEVELS.indexOf(v) >= 0) return v; } catch (_) {}
    return 'engineer';
  }
  function set(level) {
  ...
  function caps(level) { return CAPS[level || get()] || CAPS.engineer; }
  ...
  function describe() {
    ...
    return { level: lvl, index: LEVELS.indexOf(lvl), caps: caps(lvl),
      summary: { assist: 'advice only', build: 'do what I name', engineer: 'decide within the contract + repair',
        autopilot: 'plan → build → test → repair loop', godmode: 'everything incl. deploy + release' }[lvl] };
  }
