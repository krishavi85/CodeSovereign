...
2. **`Engine.Ledger.build()`** checks every criterion against the `.sovereign/`
   ...
   count).
3. **`Engine.DoD.evaluate()`** → `definition-of-done.json`: 11 criteria (incl. architecture/layering, privacy/PII and WCAG accessibility gates). It
...
5. Evidence ledger + **Definition-of-Done gate: all 11 criteria PASS** →
   `release-certificate.md` = **SOVEREIGN VERIFIED**.
