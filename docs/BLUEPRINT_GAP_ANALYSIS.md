...
| 37 | Release engineering (bump / changelog / tag / sign / checksums / notes / publish) | ✅ (offline) | `dist/engine.release.js` (`Engine.Release`). From the contract + the DoD verdict + the recorded assumptions: **`CHANGELOG.md`** (Keep-a-Changelog — Added from the mandatory scope, Verification from the DoD, Known-limitations from assumptions + excluded/unsafe + degraded), **`RELEASE_NOTES.md`** (a human announcement with the SOVEREIGN VERIFIED / PARTIAL status + how to run), **`scripts/release.js`** (dependency-free — version bump, CHANGELOG stamp, `git tag`, and **real SHA-256 checksums** via `node:crypto` of the shippable files; dry-run by default, never pushes), and **`.github/release.yml`** (auto-notes categories). `test/release-cicd.test.js` runs `release.js` for real (dry-run + `--write`). Signing + `npm publish` / GitHub-release upload still need the user's credentials, by design. |
