...
(`node test/run.js` 953, `acceptance` 38, `acceptance:build` 26,
...
| 36 | Installer engineering (install/upgrade/repair/uninstall/silent/rollback + verification) | 🟨→✅ (build outputs) | NSIS assisted installer + portable + differential `.blockmap`. `npm run installer:verify` (`scripts/verify-installer.js`) checks the build outputs automatically: every artifact's **size + sha512 match `latest.yml`**, the blockmap is present, `win-unpacked/` carries the exe + `resources/app.asar`, and the asar contains `electron/main.js` + `dist/index.html` with a version that matches `latest.yml`; it then prints the install→run→upgrade→uninstall lifecycle checklist that still needs a real Windows session. |
